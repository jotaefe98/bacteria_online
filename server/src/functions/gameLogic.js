...
exports.isColorCompatible = isColorCompatible;
exports.canPlayContagion = canPlayContagion;
exports.calculateOrganStatus = calculateOrganStatus;
exports.canPlayCard = canPlayCard;
exports.canPlayTreatment = canPlayTreatment;
exports.applyCardEffect = applyCardEffect;
exports.applyTreatmentEffect = applyTreatmentEffect;
exports.checkWinCondition = checkWinCondition;
exports.checkWinConditionForAllPlayers = checkWinConditionForAllPlayers;
exports.getPlayableCards = getPlayableCards;
exports.rebuildDeck = rebuildDeck;
// Function to verify if a color is compatible (including rainbow)
function isColorCompatible(cardColor, targetColor) {
    return (cardColor === "rainbow" ||
        targetColor === "rainbow" ||
        cardColor === targetColor);
}
// Function to check if contagion can be played (has valid targets)
function canPlayContagion(currentBoard, allBoards, currentPlayerId) {
    // Check if player has infected organs
    const infectedOrgans = Object.entries(currentBoard.organs).filter(([_, organ]) => organ.bacteria.length > 0);
    if (infectedOrgans.length === 0) {
        return { canPlay: false, reason: "You have no infected organs to spread" };
    }
    const otherPlayerIds = Object.keys(allBoards).filter((id) => id !== currentPlayerId);
    ...
    for (const [organColor, organState] of infectedOrgans) {
        if (organState.bacteria.length > 0) {
            ...
            const hasValidTargets = otherPlayerIds.some((playerId) => {
                ...
                const compatibleOrgans = Object.entries(targetBoard.organs).filter(([targetColor, targetOrgan]) => {
                    const isColorCompatible = targetColor === organColor ||
                        targetColor === "rainbow" ||
                        organColor === "rainbow";
                    const isHealthyAndFree = targetOrgan.bacteria.length === 0 &&
...
function calculateOrganStatus(organState) {
...
function canPlayCard(card, playerBoard, targetBoard, targetOrganColor) {
    switch (card.type) {
        case "organ":
...
function canPlayTreatment(card, allBoards, currentPlayerId, action) {
    ...
    switch (card.color) {
        ...
        case "transplant":
            ...
            if (!(action === null || action === void 0 ? void 0 : action.targetPlayerId) ||
                !(action === null || action === void 0 ? void 0 : action.targetOrganColor) ||
                !(action === null || action === void 0 ? void 0 : action.secondTargetPlayerId) ||
                !(action === null || action === void 0 ? void 0 : action.secondTargetOrganColor)) {
                return {
                    canPlay: false,
        ...
        case "organ_thief":
            ...
            if (!(action === null || action === void 0 ? void 0 : action.targetPlayerId) || !(action === null || action === void 0 ? void 0 : action.targetOrganColor)) {
                return {
                    canPlay: false,
...
function applyCardEffect(card, playerBoard, targetBoard, targetOrganColor, allBoards) {
    switch (card.type) {
        case "organ":
...
function applyTreatmentEffect(card, allBoards, currentPlayerId, action) {
    switch (card.color) {
        case "transplant":
...
function applyTransplant(allBoards, currentPlayerId, action) {
    ...
    const firstPlayerColors = Object.keys(firstBoard.organs).filter((c) => c !== action.targetOrganColor);
    const secondPlayerColors = Object.keys(secondBoard.organs).filter((c) => c !== action.secondTargetOrganColor);
    if (secondOrgan.organ.color !== "rainbow" &&
    ...
    if (firstOrgan.organ.color !== "rainbow" &&
...
function applyOrganThief(allBoards, currentPlayerId, action) {
    ...
    if (organToSteal.organ.color !== "rainbow" &&
...
function applyContagion(allBoards, currentPlayerId) {
    const currentBoard = allBoards[currentPlayerId];
    const infectedOrgans = Object.entries(currentBoard.organs).filter(([_, organ]) => organ.bacteria.length > 0);
    if (infectedOrgans.length === 0) {
        return { success: false, reason: "You have no infected organs to spread" };
    }
    const contagionResults = [];
    const otherPlayerIds = Object.keys(allBoards).filter((id) => id !== currentPlayerId);
    ...
    infectedOrgans.forEach(([organColor, organState]) => {
        if (organState.bacteria.length > 0) {
            ...
            const validTargets = otherPlayerIds.filter((playerId) => {
                ...
                const compatibleOrgans = Object.entries(targetBoard.organs).filter(([targetColor, targetOrgan]) => {
                    const isColorCompatible = targetColor === organColor ||
                        targetColor === "rainbow" ||
                        organColor === "rainbow";
                    const isFree = targetOrgan.status === "healthy" &&
            ...
            if (validTargets.length > 0) {
                const randomTargetPlayerId = validTargets[Math.floor(Math.random() * validTargets.length)];
                ...
                if (compatibleOrgans.length > 0) {
                    const [targetOrganColor, targetOrgan] = compatibleOrgans[Math.floor(Math.random() * compatibleOrgans.length)];
                    // Transfer bacteria
...
function applyMedicalError(allBoards, currentPlayerId, action) {
    if (!action.targetPlayerId && !action.targetOrganColor) {
        return {
            success: false,
...
function checkWinCondition(playerBoard) {
    const healthyOrgans = Object.values(playerBoard.organs).filter((organ) => {
        const isHealthy = organ.status === "healthy" ||
...
function checkWinConditionForAllPlayers(allBoards) {
...
function getPlayableCards(hand, playerBoard, allBoards) {
...
function rebuildDeck(baseDeck, hands, boards, discardPile) {
