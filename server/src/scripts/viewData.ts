  ...
  try {
    ...
    if (recentGames.length === 0) {
      ...
      recentGames.forEach((game, index) => {
        ...
        console.log(`${index + 1}. ${date} - ${game.playerCount} jugadores, ${duration} min, Ganador: ${game.winnerName}`);
      });
    }
    ...
    if (playerStats.length === 0) {
      ...
      playerStats.slice(0, 5).forEach((player, index) => {
        ...
        console.log(`${index + 1}. ${player._id}: ${player.gamesPlayed} juegos, ${player.wins} victorias (${winRate}% win rate)`);
      });
    }
    
  } catch (error) {
    console.error('❌ Error conectando a MongoDB:', error);
    console.log('💡 Verifica que tu archivo .env tenga la URL correcta de MongoDB');
  }
}
