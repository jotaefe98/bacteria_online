...
export const useGameSounds = () => {
  ...
  const playSound = useCallback(
    async (soundType: SoundType, volumeMultiplier: number = 1) => {
      ...
      try {
        ...
        if (currentAudioRef.current) {
          ...
          if (isIOS() && currentAudioRef.current.parentNode) {
            currentAudioRef.current.parentNode.removeChild(
        ...
        // MISMO VOLUMEN para todos los sonidos (con multiplicador opcional)
        audio.volume = masterVolume * volumeMultiplier;
